import sys
Rosalinds = open("Rosalinds.txt", "r")
count = 0
line = ""
fullLine = ""
toSnip = ""
        ...
        if (count < 2):
            if (y == ">"):
                ...
                if (y == ">"):
                    count = count+1
                    break
                fullLine = fullLine.strip() + line.strip()
        #        print (fullLine)
                break
        elif (count > 1):
            if ( y == ">"):
                ...
                toSnip = line
                fullLine = fullLine.replace(toSnip.strip(), '')
        #        print fullLine
                break

##make into RNA from Dictionary
DNA = fullLine
index = 0
char = "U"
for x in DNA:
    if x == "T":
        DNA = DNA[:index] + char + fullLine[index +1:]
    index = index +1
RNA = DNA


##Previously coded Decode function
def decode(RNA):
  n=3
  ...
  for codon in x:
    ...
    sys.stdout.write(proteinMade(codon))  #comma makes it all on one line
